// Vercel Serverless Function — Gemini AI Summary
// Deploy: set GEMINI_API_KEY in Vercel project environment variables
...
export default async function handler(req, res) {
  ...
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'อนุญาตเฉพาะ POST method เท่านั้น' });
  }

  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    return res.status(500).json({
      error: 'ไม่พบ GEMINI_API_KEY กรุณาตั้งค่า Environment Variable ใน Vercel Dashboard'
    });
  }
...
${accidentList}`;
  ...
  try {
    ...
    if (!geminiRes.ok) {
      ...
      return res.status(500).json({ error: `Gemini API ตอบกลับผิดพลาด: ${geminiRes.status}` });
    }
    ...
    if (!summary) {
      return res.status(500).json({ error: 'ไม่สามารถรับข้อมูลสรุปจาก Gemini ได้' });
    }
